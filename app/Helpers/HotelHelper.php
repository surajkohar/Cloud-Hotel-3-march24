...
//     return  $formattedDate;
    ...
    function extractImportantFacilities($facilities) {
        $importantFacilities = [
            ...
            '24-hour front desk', 'Breakfast available', 'Laundry facilities', 'Elevator','laundry service','InternetAccess',
            ...
            'Guests must provide proof of COVID-19 vaccination','Restaurants','Lift','Shower','Gym','Satellite TV',' non-smoking area'
        ];
    ...
    function formatFacility($facilityName) {
        ...
        if (strpos(strtolower($facilityName), 'internet') !== false) {
            $printedFacilities[] = $facilityName;
            return '<i class="fa-solid fa-wifi  text-[#DC2626] mr-2"></i>' .    $facilityName;
        }
        ...
        if (stripos(strtolower($facilityName), 'bars') !== false || stripos(strtolower($facilityName), 'lounges') !== false || stripos(strtolower($facilityName), 'restaurants') !== false) {
            $printedFacilities[] = $facilityName;
        ...
        if (strpos(strtolower($facilityName), 'shower') !== false){
            $printedFacilities[] = $facilityName;
            return '<i class="fa-solid fa-shower  text-[#DC2626] mr-2"></i>' .    $facilityName;
        }
        if (strpos(strtolower($facilityName), 'laundry') !== false){
            $printedFacilities[] = $facilityName;
            return '<i class="fa-solid fa-shirt text-[#DC2626] mr-2"></i>' .    $facilityName;
        }
        if (strpos(strtolower($facilityName), 'laundry service') !== false){
            $printedFacilities[] = $facilityName;
            return '<i class="fa-solid fa-shirt text-[#DC2626] mr-2"></i>' .    $facilityName;
        }
        if (strpos(strtolower($facilityName), 'elevator') !== false || strpos(strtolower($facilityName), 'lift') !== false){
            $printedFacilities[] = $facilityName;
            return '<i class="fa-solid fa-elevator text-[#DC2626] mr-2"></i>' . $facilityName;
        }
//        if (str_contains(strtolower($facilityName), ['elevator', 'lift'])) {
//            $printedFacilities[] = $facilityName;
//            return '<i class="fa-solid fa-elevator text-[#DC2626] mr-2"></i>' . $facilityName;
//        }
        ...
        if (strpos(strtolower($facilityName), 'non-smoking area') !== false){
            $printedFacilities[] = $facilityName;
            return '<i class="fa-solid fa-ban-smoking text-[#DC2626] mr-2"></i>' .    $facilityName;

        }
        if (strpos(strtolower($facilityName), 'satellite tv') !== false){
            $printedFacilities[] = $facilityName;
            return '<i class="fa-solid fa-satellite-dish text-[#DC2626] mr-2"></i>' .    $facilityName;
        ...
        if (strpos(strtolower($facilityName), 'fitness ') !== false || strpos(strtolower($facilityName), 'gym ') !== false){
            $printedFacilities[] = $facilityName;
