...
<!-- TimerComponent.vue -->
<template>
    <div :key="key" class="w-full mx-auto bg-white p-4 rounded-md shadow-md text-center text-gray-800">
        ...
        <p class="text-xl text-red-500" v-if="sessionExpired" :class="{ 'blink': seconds < 5 }">
            Wooops !! Session is going to expired in {{ minutes }}:{{ seconds < 10 ? '0' : '' }}{{ seconds }}
        </p>
    </div>
</template>
...
localStorage.sessionStartedTime = null;
this.redirectToPage();
