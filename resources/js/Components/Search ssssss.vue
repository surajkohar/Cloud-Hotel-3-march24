...
<div class="hotel-search p-4 bg-gray-100 ">
    ...
    <div v-show="showloader" class="z-40 fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-70" id="loading-spinner">
    ...
    />
    <div v-if="filteredCountries.length > 0" class="suggestion-container">
        <div v-for="(country, index) in filteredCountries" :key="index" required class="suggestion-item" @click="selectCountryFromSuggestions(country)">
            {{ country.name }}
        </div>
    </div>
        </div>
        ...
        />
        <div v-if="filteredCities.length > 0" class="suggestion-container">
            <div v-for="(city, index) in filteredCities" :key="index" class="suggestion-item" @click="selectCityFromSuggestions(city)">
            {{ city.CityName }}
            </div>
        </div>
        </div>

        <div class="form-group lg:w-[21%] md:w-[50%] w-[100%] mb-4 px-2">
            <label for="checkInDate" class="block text-sm font-medium text-gray-700">Check-in & Check-out Date</label>
            <VueDatePicker :format="'yyyy-MM-dd'"     :styles="customDateStyles"  :min-date="getCurrentDate()"   type="date" placeholder="2024-01-16-2024-01-20" id="checkInDate" v-model="date" range multi-calendars class="bg-white border border-gray-200 text-black text-sm rounded-md focus:ring-0 focus:border-none  w-full py-1.5 flex justify-start px-3" />
        </div>
        ...
        <div class=" form-group lg:w-[18%] md:w-[50%] w-[100%] mb-4 px-2 relative">
            ...
            <button @click="openGuestModal()" type="button" class="bg-white border border-gray-200 text-black text-sm rounded-md focus:ring-0 focus:border-none block w-full py-3.5 flex justify-start px-3">

                {{ selectedRooms }} Room{{ selectedRooms > 1 ? 's' : '' }},{{  rooms[0].numberofAdults }} Adults,{{ rooms[0].numberOfChildren }} Child
            </button>
            <div v-show="isGuestModalOpen" id="guestModal" class="w-72 h-max bg-gray-100 rounded-md absolute left-0 z-50  shadow-md shadow-gray-300 ">
                <div class="w-full flex justify-between p-6 border-b-2 border-b-gray-300 ">
                    ...
                    <select id="rooms" v-model="selectedRooms" @change="updateRoomsContent" class="bg-white border border-gray-200 text-black text-sm rounded-md focus:ring-0 focus:border-none block w-32 py-1.5 px-3">
                ...
                <div class="w-full py-2 px-6 ">
                        ...
                        <div v-for="(room, index) in rooms" :key="index">
                            <div class="w-full py-2 px-6">
                                ...
                                <div class="w-full flex justify-between py-2 px-6 border-b-2  border-b-gray-300">
                                    ...
                                    <select id="adults" v-model="room.numberofAdults" class="bg-white border border-gray-200 text-black text-sm rounded-md focus:ring-0 focus:border-none block w-32 py-1.5 px-3">
                                ...
                                <div class="w-full flex justify-between py-2 px-6 border-b-2  border-b-gray-300">
                                    ...
                                    <select id="numberOfChildren" v-model="room.numberOfChildren" @change="updateChildAges(index)" class="bg-white border border-gray-200 text-black text-sm rounded-md focus:ring-0 focus:border-none block w-32 py-1.5 px-3">
                                ...
                                <div class="w-full grid grid-cols-2 gap-3 py-2 px-6 border-b-2 border-b-gray-300">
                                    <div v-show="room.numberOfChildren>0" v-for="(childAge, childIndex) in room.childAges" :key="childIndex">
                                        <span class="text-gray-600 font-semibold text-sm">Child Age</span>
                                        <select id="numberOfChildren" required v-model="room.childAges[childIndex]" class="bg-white border border-gray-200 text-black text-sm rounded-md focus:ring-0 focus:border-none block w-full py-1.5 px-2" placeholder="Age">
                                            <option v-for="i in 12" :key="i" :value="i">{{ String(i).padStart(2, '0') }}</option>
                    ...
                    <div class="w-full flex justify-center gap-3 py-2 px-2 border-b-2 border-b-gray-300">
                        <button @click="closeGuestModal()" type="button" class="rounded-md bg-sky-500 text-white w-full flex justify-center py-1.5 font-semibold text-lg">Done</button>
                    </div>
                </div>
            </div>
        </div>
        <div class=" lg:w-[15%] md:w-[50%] w-[100%] mb-4 px-2 flex items-center mt-[12px] ml-8">
...
export default {
 ...
 : [{
    ...
    watch: {
        date(){
            console.log('date iss', this.date);
...
async fetchedSelectedLocation(searchParams) {
    ...
    const countryDetails = await axios.get('/api/getsinglecity/' + searchParams.city);
    ...
    this.searchLocation =countryDetails.data.cityName;
     ...
     // Manually update the input value using refs
    //  this.$nextTick(() => {
    //     this.$refs.countryInput.value = countryName;
    // });
        // Fetch cities for the selected country
        this.fetchCities();
  ...
  selectCityFromSuggestions(city) {
    ...
    this.filteredCities = [];
        ...
        search(e) {
            const roomDetails = encodeURIComponent(JSON.stringify(this.rooms));
            // e.preventDefault();
            // var roomDetails = JSON.parse(this.rooms);
